...
- **`attr:` prefix / 前缀** — Output as HTML attributes. `attr:value="init"` becomes `value="init"`.
- **`attr:` 前缀** — 作为 HTML 属性输出。`attr:value="init"` 输出为 `value="init"`。
- **`prop:` prefix / 前缀** — Ignored. These are only meaningful for client-side DOM property assignment.
- **`prop:` 前缀** — 忽略。仅对客户端 DOM property 赋值有意义。
- **Event handlers / 事件处理器** — Ignored. `onClick`, `onInput`, etc. are not serialized.
